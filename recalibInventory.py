...
def recalib_inventory(acc, curr_inv):
  ...
  # curr_inv and new_inv are mutable since they are both dicts; makes a copy of curr_inv so changes to new_inv dont affect curr_inv
  ...
  while True:
    ...
    if item_name == "exit":
        ...
        if item_container_value == "exit":
            save_bool = input(
                "Do you want to save changes? (this particular item will not be changed regardless since you have not entered a value yet) (y/n): ").lower()
            if save_bool == "n":
                return curr_inv
            else:
                return new_inv
