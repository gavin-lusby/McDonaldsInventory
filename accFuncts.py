# accFuncts is short for Account Functions There are two types of user accounts ("accs"): manager accounts and
# employee accounts. Both have different levels of access to certain functionalities within the program
...
def print_menu(manager_status):
    # Saves menu items according to manager or employee status (ex. employees can only access employee actions,
    # not manager-only actions)
...
def acc_menu(manager_status, user_name, valid_accs):
    # This function prints and allows users to choose from the account settings menu according to their status as
    # manager or employee
...
def change_pass(manager_status, user_name, valid_accs):
    # This function allows account passwords to be changed Side note: We, the developers, unanimously agree that it's
    # annoying when you forget your current password, then attempt to change your password and get hit with an error
    # message telling you that your new password cannot be your current password (we're all a little forgetful
    # sometimes 😔). Thus, we did not include this annoying feature. We hope you can understand our decision on this
    # very important matter.

    # Allows users to change their account (or another account's, in the case of managers making the password
    # changes) password if they know their account's current password
    ...
    # Users can only change an account's password if they know their own account's password (extra security measure
    # in case an employee has accidentally left their account logged in after initially entering their password at
    ...
    while user_name not in valid_accs:
        user_name = str(input(
            "\nThis user does not exist. Please enter the correct full name of the existing user whose account you "
            "would like to remove or type \"exit\": ")).upper()
...
def create_acc_pass():
    # Note: Appended to new_acc so changes can be made to users.csv, in addition to returning values from new_acc or
    # None (this allows us to check if the user has saved changes or not at every step)
    ...
    while remove_acc not in valid_accs or remove_acc == user_name:
        if remove_acc == user_name:
            ...
            remove_acc = str(input(
                "\nPlease enter the correct full name of another existing user whose account you would like to remove "
                "or type \"exit\": ")).upper()
            ...
            remove_acc = str(input(
                "\nThis user does not exist. Please enter the correct full name of the existing user whose account "
                "you would like to remove or type \"exit\": ")).upper()
